...
import {ILocation, INodeDataModel, NavigationRequest} from "..";
...
export class FloorDataManager{
    ...
    public _getClosestNode(locations: ILocation) :INodeDataModel | null {
        ...
        let sortedList = this._sortNodesFromPosition(nodes
            .filter((node) => node.includedFloorId === locations.floorId ), locations.position!); // 출발지 노드 후보지 1개로 변경
    ...
    public _getObjectsByNodeId(floorId : string, nodeId : string) : IObjectDataModel[] | []{
        ...
        node.objectIds.forEach(objectId => {
            const idx = floor.objects
                .findIndex(o => o.id === objectId);
    ...
    public _isContain(model: FormalModel, position: IPosition): boolean {
        ...
        for (let frontIndex = 0; frontIndex < sizeOfVertexes; frontIndex++) {
            ...
            if (frontPoint.y < y && followPoint.y >= y
                || followPoint.y < y && frontPoint.y >= y) {
                /**
                 ...
                 *      M = (y2 - y1) / (x2 - x1)
                 */
                if (frontPoint.x + (y - frontPoint.y) / (followPoint.y - frontPoint.y)
                    * (followPoint.x - frontPoint.x) < x) {
    ...
    public _isTransObject(obj : IObjectDataModel) : boolean{
        ...
        if (obj.attributeCode === Constants.TRANSCODE.ELEVATOR ||
